<template>
  <div>
    <el-form ref="detail" :model="detail" :rules="rules" label-width="100px" :disabled="disabled">
      <el-row>
        <el-col :span="12">
          <el-form-item label="摊销月度;YYYY-MM" prop="month">
            <el-input v-model="detail.month" placeholder="请输入摊销月度;YYYY-MM" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
...
<script>
  ...
  import {getBudgetDesignFee, delBudgetDesignFee, addBudgetDesignFee, updateBudgetDesignFee} from "@/api/cost/budgetDesignFee";

  export default {
    name: 'budgetDesignFeeUpdate',
    components: {},
    inject: ['handleQuery'],
    data() {
      return {
        ...
        rules: {
          month: [
            { required: true, message: "摊销月度;YYYY-MM不能为空", trigger: "blur" }
          ],          itemNo: [
    ...
    created() {
      this.getDetail(this.budgetDesignFeeId)
    },
    methods: {
      /** 表单重置 */
      reset() {
      ...
      /** 获取研发投入新品设计拟定详情 */
      getDetail(budgetDesignFeeId) {
        if (isNullOrEmpty(budgetDesignFeeId)) {
          this.reset()
        } else {
          getBudgetDesignFee(budgetDesignFeeId).then(response => {
        ...
        this.$refs["detail"].validate(valid => {
          if (valid) {
            if (this.detail.id != null) {
              updateBudgetDesignFee(this.detail).then(response => {
              ...
              addBudgetDesignFee(this.detail).then(response => {
                if (response.code === 200) {
                  this.msgSuccess(response.msg)
                  this.handleClose()
                  this.handleQuery()
